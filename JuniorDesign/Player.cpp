...
PlayerMove Player::move()
{
	...
	} else 
	{
		...
		std::cout << this->currentOut 
			<< " used " 
			<< this->team[this->currentOut].getMove(i)
			<< std::endl;
	}
	return move;
}

Pokemon Player::getPokemon(std::string name) {
...
void Player::buildTeam(std::string filepath)
{
	...
	while (std::getline(infile, line))
	{
		...
		if (flagPokename) {
			pokename = line;
			this->team.insert(std::pair<std::string, Pokemon>(pokename, *new Pokemon(pokename)));
			flagPokename = false;
		} else { 
